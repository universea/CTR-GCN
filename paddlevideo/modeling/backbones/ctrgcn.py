...
class CTRGC(nn.Layer):
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        ...
        # print('CTRGC----rel_reduction=', rel_reduction)
        # print('CTRGCin_channels=', in_channels)
        # print('CTRGCrel_channels=', self.rel_channels)
        # print('CTRGCout_channels=', self.out_channels)
    ...
    def forward(self, x, A=None, alpha=1):
        # print('CTRGC-forward:',x.shape)
        # print('CTRGC-A-forward:',A.shape)
...
class unit_gcn(nn.Layer):
    ...
    def forward(self, x):
        # print('unit_gcn-forward:',x.shape)
        y = None
        if self.adaptive:
            A = self.PA
        else:
            A = self.A#.cuda(x.get_device())
        # print('unit_gcn-forward-A:',A.shape)
...
class CTRGCN(nn.Layer):
    ...
    def forward(self, x):
        ...
        #print('CTRGCN-forward:',x.shape) # [32, 2, 350, 25, 1]

        x = x.transpose((0, 4, 3, 1, 2))  # N, M, V, C, T
        #print('CTRGCN-transpose:',x.shape) # [32, 1, 25, 2, 350]

        x = x.reshape((N * M, V * C, T))
        #print('CTRGCN-reshape:',x.shape) # [32, 50, 350]

        if self.data_bn:
            x.stop_gradient = False
        x = self.data_bn(x)
        #print('CTRGCN-data_bn:',x.shape) # [32, 50, 350]

        x = x.reshape((N, M, V, C, T))
        #print('CTRGCN-reshape:',x.shape) # [32, 1, 25, 2, 350]

        x = x.transpose((0, 1, 3, 4, 2))  # N, M, C, T, V
        #print('CTRGCN-transpose:',x.shape) # [32, 1, 2, 350, 25]

        x = x.reshape((N * M, C, T, V)) 
        #print('CTRGCN-normalization-forward:',x.shape) #[32, 2, 350, 25]
